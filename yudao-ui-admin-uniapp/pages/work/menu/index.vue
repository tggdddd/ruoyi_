		...
		<scroll-view class="scroll" scroll-y @scrolltolower="loadMore()">
			...
			<uni-table ref="table" :loading="loading" border stripe emptyText="暂无更多数据"
				...
				<uni-tr v-for="(row, index) in tableData" :key="index">
					<!-- <uni-td>{{row.parentId}}</uni-td> -->
					<uni-td>{{row.name}}</uni-td>
					<uni-td><view v-dictData="{type:'system_menu_type',value:row.type}"></view></uni-td>
					<uni-td>{{row.path}}</uni-td>
					<uni-td>{{row.component}}</uni-td>
					<uni-td>{{row.componentName}}</uni-td>
					<uni-td>{{row.permission}}</uni-td>
					<uni-td>
	...
	export default {
		...
		methods: {
			loadMore() {
				...
				getMenuListApi(this.queryParams).then(response => {

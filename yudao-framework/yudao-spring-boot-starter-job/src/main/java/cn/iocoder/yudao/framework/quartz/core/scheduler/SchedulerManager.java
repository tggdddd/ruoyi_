...
public class SchedulerManager {
    ...
    public void addJob(Long jobId, String jobHandlerName, String jobHandlerParam, String cronExpression,
                       Integer retryCount, Integer retryInterval,String jobKey,String triggerKey)
            throws SchedulerException {
        JobDetail jobDetail = JobBuilder.newJob(JobHandlerInvoker.class)
                    .usingJobData(JobDataKeyEnum.JOB_ID.name(), jobId)
                    .usingJobData(JobDataKeyEnum.JOB_HANDLER_NAME.name(), jobHandlerName)
        ...
        // 创建 Trigger 对象
        Trigger trigger = this.buildTrigger(triggerKey, jobHandlerParam, cronExpression, retryCount, retryInterval);
        // 新增调度
        scheduler.scheduleJob(jobDetail, trigger);
    }
    /**
     ...
     */
    public void updateJob(String triggerKey, String jobHandlerParam, String cronExpression,
                          Integer retryCount, Integer retryInterval)
            throws SchedulerException {

        // 创建新 Trigger 对象
        Trigger newTrigger = this.buildTrigger(triggerKey, jobHandlerParam, cronExpression, retryCount, retryInterval);
        // 修改调度
        scheduler.rescheduleJob(new TriggerKey(triggerKey), newTrigger);
    }
    public void updateOrInsertTrigger(String jobKey,String triggerKey, String jobHandlerParam, String cronExpression,
                          Integer retryCount, Integer retryInterval)
            throws SchedulerException {
        // 创建新 Trigger 对象
        Trigger newTrigger = this.buildTrigger(triggerKey, jobHandlerParam, cronExpression, retryCount, retryInterval);
        if (scheduler.getTrigger(TriggerKey.triggerKey(triggerKey)) != null) {
            // 修改调度
            scheduler.rescheduleJob(new TriggerKey(triggerKey), newTrigger);
        }else {
            // 新增调度
            scheduler.scheduleJob(scheduler.getJobDetail(JobKey.jobKey(jobKey)), newTrigger);
        }
    }
    /**
     * 删除 Quartz 中的 Job
    ...
    public void deleteTrigger(String triggerKey) throws SchedulerException{
        if(scheduler.getTrigger(TriggerKey.triggerKey(triggerKey)) != null) {
    ...
    public void resumeJob(String jobHandlerName,String triggerKey) throws SchedulerException {
        scheduler.resumeJob(new JobKey(jobHandlerName));
    ...
    private Trigger buildTrigger(String triggerKey, String jobHandlerParam, String cronExpression,
                                 Integer retryCount, Integer retryInterval) {

		...
		<scroll-view class="scroll" scroll-y @scrolltolower="loadMore()">
			...
			<uni-table ref="table" :loading="loading" border stripe emptyText="暂无更多数据"
				...
				<uni-tr v-for="(row, index) in tableData" :key="index">
					<uni-td>{{row.title}}</uni-td>
					<uni-td><view v-dictData="{type:'system_notice_type',value:row.type}"></view></uni-td>
					<uni-td>
	...
	export default {
		...
		methods: {
			loadMore() {
				...
				getNoticePage(this.queryParams).then(response => {

		...
		<scroll-view class="scroll" scroll-y @scrolltolower="loadMore()">
			...
			>
				...
				<uni-tr v-for="(row, index) in tableData" :key="index">
					...
					<uni-td>
					<view class="button" @click="viewData(row.type)">{{row.type}}</view>
					</uni-td>
		<uni-td>
...
<script>
	...
	export default {
		...
		methods: {
			// 查看数据
			viewData(type){
				this.$tab.navigateTo('/pages/work/dict/data?type='+type)
			},
			loadMore() {
